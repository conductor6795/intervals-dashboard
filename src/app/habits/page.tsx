...
import {
  ...
  RefreshCw, Download, Bell, CheckSquare,
} from "lucide-react";
...
/* ══════════════════════════════════════════
   SUB-COMPONENTS
══════════════════════════════════════════ */
function HabitGrid({ habit, history, today }: { habit: Habit; history: History; today: string }) {
...
export default function HabitsPage() {
  ...
  /* ── Schedule notifications + auto-sync ── */
  useEffect(() => {
  ...
  const exportCSV = () => {
    ...
    download(`habits-${today}.csv`, [heads.join(","), ...rows].join("\n"), "text/csv");
  };
  const exportJSON = () => download(`habits-${today}.json`, JSON.stringify({ habits, history }, null, 2), "application/json");
        ...
        <div className="flex items-center gap-1 flex-wrap">
          {(["today","stats","manage","settings"] as const).map(t => {
            ...
            return (
              <button
                key={t}
                onClick={() => { setTab(t); setForm(null); setDelId(null); }}
                className={clsx(
                  "text-xs px-3 py-1.5 rounded-xl border transition-colors",
                  tab === t
                    ? "text-white border-transparent"
                    : "text-dash-muted border-dash-border hover:text-white hover:border-indigo-500/50"
                )}
          ...
          <>
            ...
            <div className="flex items-center justify-between p-3 rounded-2xl border border-dash-border bg-dash-card">
              ...
              <div className="text-center">
                ...
                <p className="text-sm font-medium text-white">{selLabel.dat}</p>
              </div>
            ...
            {habits.length === 0 && (
              <div className="rounded-2xl border border-dashed border-dash-border p-10 text-center text-dash-muted text-sm">
                Noch keine Habits.<br />
                Im Tab <strong className="text-white">Verwalten</strong> anfangen.
              </div>
            )}
            ...
            {habits.map(hb => {
              ...
              return (
                <div
                  key={hb.id}
                  className={clsx(
                    "flex items-center gap-3 p-3.5 rounded-2xl border bg-dash-card transition-colors",
                    hb.habitType === "checkbox" && "cursor-pointer select-none",
                  )}
                ...
                >
                  {/* Check circle */}
                  <div
                    className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 text-base transition-all"
                  ...
                  {hb.habitType === "numeric" && (
                    <div className="flex items-center gap-1.5 flex-shrink-0" onClick={e => e.stopPropagation()}>
                      <button
                        onClick={() => setNum(hb.id, Math.max(0, Math.round(((numVal??0)*10-5))/10))}
                        className="w-7 h-7 rounded-lg border border-dash-border text-dash-muted hover:text-white text-base flex items-center justify-center transition-colors"
                      >−</button>
                      <input
                        type="number" step="0.5" min="0"
                        value={numVal ?? ""} placeholder="0"
                      ...
                      />
                      <button
                        onClick={() => setNum(hb.id, Math.round(((numVal??0)*10+5))/10)}
                        className="w-7 h-7 rounded-lg border border-dash-border text-dash-muted hover:text-white text-base flex items-center justify-center transition-colors"
                      >+</button>
          ...
          <>
            <div className="flex gap-2">
              {(["stats","corr"] as const).map(s => (
                ...
                >
                  {s==="stats" ? "Statistiken" : "Korrelation"}
                </button>
              ))}
            </div>
            ...
            {statsSub === "stats" && (
              <>
                ...
                {habits.map(hb => {
                  ...
                  const rate   = Math.round(c28/28*100);
                  const streak = calcStreak(hb, history);
                  const wc     = calcWeekCount(hb, history);
                  const ld     = calcLastDone(hb, history);
                  ...
                  return (
                    <div key={hb.id} className="rounded-2xl border border-dash-border bg-dash-card p-4">
                      ...
                      <HabitGrid habit={hb} history={history} today={today} />
              ...
              <>
                ...
                <div className="rounded-2xl border border-dash-border bg-dash-card p-4">
                  ...
                  <div className="flex gap-2 mb-2 ml-24">
                    {DOW.map(d => <div key={d} className="w-5 text-[9px] text-dash-muted/40 text-center">{d}</div>)}
                  </div>
                  {habits.map(hb => {
                      ...
                      <div key={hb.id} className="flex items-center gap-2 mb-2">
                        ...
                        <div className="flex gap-1">
                          {DOW.map((_, di) => {
                            let done=0, total=0;
                            for (let w=0; w<12; w++) {
                              const back = (currDow-di+7)%7+w*7;
                              const ds   = daysAgo(back);
                              if (ds <= today) { total++; if (isCompleted(hb, ds, history)) done++; }
                            }
                            const pct = total > 0 ? done/total : 0;
                            return <div key={di} className="w-5 h-5 rounded-sm" style={{ background:hb.color, opacity:0.08+pct*0.92, border:"0.5px solid rgba(30,45,74,0.8)" }} title={`${DOW[di]}: ${Math.round(pct*100)}%`} />;
                          })}
                        </div>
                      </div>
                    );
          ...
          <>
            ...
            <div className="rounded-2xl border border-dash-border bg-dash-card p-4 space-y-3">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <button
                  onClick={async () => { setSyncMsg("Synchronisiere…"); const ok = await doSync(today); setSyncMsg(ok ? `✓ Sync: ${new Date().toLocaleString("de-DE")}` : "✗ Fehlgeschlagen — API-Key / ID prüfen"); }}
            ...
            <div className="rounded-2xl border border-dash-border bg-dash-card p-4 space-y-3">
              ...
              {typeof window !== "undefined" && "Notification" in window && (
                Notification.permission === "granted" ? (
                  ...
                  <button onClick={async()=>{const p=await Notification.requestPermission();if(p==="granted")setSettings(s=>({...s,notifEnabled:true}));}} className="flex items-center gap-2 text-xs px-3 py-1.5 rounded-xl border border-dash-border text-dash-muted hover:text-white hover:border-indigo-500/50 transition-colors">
            ...
            <div className="rounded-2xl border border-dash-border bg-dash-card p-4 space-y-3">
              ...
              <p className="text-xs text-dash-muted">Daten liegen im Browser-localStorage — nur in diesem Browser. Monatlich exportieren und in Paperless-ngx oder auf die Backup-HDD ablegen.</p>
              <div className="flex gap-2 flex-wrap">
